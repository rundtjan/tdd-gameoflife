...
export function parsePattern(data) {
  ...
  for (let i = 0; i < dimensions.y; i++) {
    result.push(new Array(parseInt(dimensions.x)));
  }
  let char = 0;
  ...
  for (let i = 0; i < board.length; i++) {
    for (let j = 0; j < board.length; j++) {
      ...
      if (surround > 3) result[i][j] = "b";
        ...
        board[i][j] === "o" ? (result[i][j] = "o") : (result[i][j] = "b");
      else result[i][j] = "b";
    }
  }
  return result;
...
export function extractPattern(board) {
  let [smallestY, smallestX, biggestY, biggestX] = [
    board.length,
    board.length,
    -1,
    -1,
...
function writeToFile(resultPattern) {
  ...
  fs.writeFileSync(
    "result.rle",
    "x = " +
      resultPattern[0].length +
      ", y = " +
      resultPattern.length +
      ", rule = B3/S23\n"
  );
  fs.writeFileSync("result.rle", rleEncoder(resultPattern), { flag: "a+" });
}

function printToScreen(board) {
  board.forEach((line) =>
    console.log(line.toString().replace(/,/gi, "").replace(/b/gi, "."))
  );
  console.log("\n\n");
}
